...
```mermaid
flowchart TD
    P["Passion for an interest"] --> C["Conviction that it is imperative"]
    C --> K["Count the cost<br/>effort · money · sacrifice"]
    K --> Q{"Still burning<br/>in your blood?"}
    Q -- "Yes" --> PL["Plunge in"]
    Q -- "No" --> W["Dream stays wishful thinking"]
    PL --> U["Uphill road"]
    U --> M["Motivated by love of the work<br/>+ prospect of success"]
    M --> S["Support network stands by you"]
    S --> A["Life changes the dream<br/>→ larger dreamscape"]
    A --> R["Dream realised (may take years)"]
```
